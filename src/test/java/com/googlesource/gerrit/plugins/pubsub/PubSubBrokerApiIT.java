...
public class PubSubBrokerApiIT extends LightweightPluginDaemonTest {
  ...
  public void shouldSendEvent() throws IOException {
    ...
    String expectedMessageJson =
        String.format(
            "{\"header\":{\"eventId\":\"%s\",\"sourceInstanceId\":\"%s\",\"eventType\":\"\"},\"body\":{\"type\":\"project-created\",\"eventCreatedOn\":%s}}",
            id, id, event.eventCreatedOn);

    EventMessage eventMessage = new EventMessage(new EventMessage.Header(id, id), event);

    objectUnderTest.send(TOPIC_ID, eventMessage);

    readAndCompareMessages(
        (pullResponse) -> {
  ...
  private void readAndCompareMessages(Consumer<PullResponse> validate) throws IOException {
